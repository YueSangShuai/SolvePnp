...
std::vector<bbox_t> TRTModule::operator()(const cv::Mat &src) const {
    ...
    for (int i = 0; i < TOPK_NUM; i++) {
        ...
        for (auto &pt : box.pts) {
            pt.x *= fx, pt.y *= fy;
            cv::circle(temp,cv::Point2f( pt.x,pt.y),5,cv::Scalar(255,0,255),5);
        }
        cv::line(temp,box.pts[0],box.pts[2],cv::Scalar(255,255,0));
        cv::line(temp,box.pts[1],box.pts[3],cv::Scalar(255,255,0));
//        cv::Rect2f bbox1;
